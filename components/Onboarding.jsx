...
const steps = [
  {
    ...
    title: "Provably fair casino games on Base",
...
export default function Onboarding({ onSkip, onComplete }) {
  ...
  return (
    <div className="bc-onboarding" role="dialog" aria-modal="true" aria-label="BaseCast onboarding">
      <style>{`
        .bc-onboarding{position:fixed;inset:0;z-index:200;display:flex;align-items:center;justify-content:center;padding:18px;background:radial-gradient(circle at 20% 10%,rgba(96,200,255,.18),transparent 32%),radial-gradient(circle at 84% 18%,rgba(255,209,102,.14),transparent 30%),rgba(4,5,12,.84);backdrop-filter:blur(22px)}
        .bc-onboarding-card{position:relative;width:min(94vw,520px);overflow:hidden;border:1px solid rgba(255,255,255,.14);border-radius:28px;background:linear-gradient(150deg,rgba(13,16,34,.96),rgba(7,5,15,.98));box-shadow:0 28px 90px rgba(0,0,0,.62),0 0 80px rgba(108,99,255,.18);animation:bcPanelIn .34s ease both}
        ...
        .bc-orb.one{width:190px;height:190px;left:-76px;top:-60px;background:radial-gradient(circle,rgba(96,200,255,.28),transparent 70%)}
        .bc-orb.two{width:170px;height:170px;right:-72px;bottom:10%;background:radial-gradient(circle,rgba(255,209,102,.20),transparent 70%);animation-delay:1.2s}
        .bc-onboarding-body{position:relative;z-index:1;padding:28px}
        .bc-logo-ring{width:86px;height:86px;margin:0 auto 18px;border-radius:24px;display:grid;place-items:center;background:linear-gradient(145deg,rgba(96,200,255,.18),rgba(108,99,255,.18),rgba(255,209,102,.16));border:1px solid rgba(255,255,255,.16);box-shadow:0 0 38px rgba(108,99,255,.25);animation:bcPulse 3s ease-in-out infinite}
        .bc-step-visual{width:78px;height:78px;margin:0 auto 22px;border-radius:24px;display:grid;place-items:center;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.055);box-shadow:inset 0 0 30px rgba(255,255,255,.035)}
        ...
        .bc-title{font-size:clamp(25px,6vw,36px);line-height:1.04;color:#F0F2FF;font-weight:900;letter-spacing:-.04em;margin-bottom:12px}
        .bc-body{font-size:14px;line-height:1.72;color:#A4A8C3;max-width:410px;margin:0 auto}
        ...
        @media(max-width:520px){.bc-onboarding{align-items:flex-end;padding:10px}.bc-onboarding-card{border-radius:24px 24px 18px 18px}.bc-onboarding-body{padding:22px}.bc-actions{grid-template-columns:1fr}.bc-logo-ring{width:76px;height:76px}.bc-step-visual{width:68px;height:68px}}
      `}</style>
