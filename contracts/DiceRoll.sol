...
contract DiceRoll is BaseGame {
    ...
    /// @notice Bet LOW (1-3) or HIGH (4-6)
    ...
    {
        return _place(wager, high ? BetType.RANGE_HIGH : BetType.RANGE_LOW, 0);
    }

    /// @notice Bet on exact number 1-6
    /// @dev Send ETH equal to getEntropyFee() as msg.value
    function placeBetExact(uint256 wager, uint8 number)
        external payable whenNotPaused nonReentrant returns (uint64)
    {
        require(number >= 1 && number <= 6, "Invalid number");
        return _place(wager, BetType.EXACT, number);
    }

    function _place(uint256 wager, BetType betType, uint8 exactNumber)
    ...
    function _resolveGame(uint64 seqNum, bytes32 randomNumber) internal override {
        ...
        bool    won;
        uint256 payoutBps_;
        if      (bet.betType == BetType.RANGE_LOW)  { won = rolled <= 3; payoutBps_ = rangePayoutBps; }
        else if (bet.betType == BetType.RANGE_HIGH) { won = rolled >= 4; payoutBps_ = rangePayoutBps; }
        else                                         { won = rolled == bet.exactNumber; payoutBps_ = exactPayoutBps; }

        uint256 payout = won ? (uint256(bet.wager) * payoutBps_) / 10_000 : 0;
    ...
    function setRangePayoutBps(uint256 _bps) external onlyOwner {
        require(_bps >= 15_000 && _bps <= 19_900, "Out of range");
        rangePayoutBps = _bps;
    }

    function setExactPayoutBps(uint256 _bps) external onlyOwner {
        require(_bps >= 50_000 && _bps <= 59_000, "Out of range");
        exactPayoutBps = _bps;
    }
}
