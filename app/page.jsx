...
export default function App() {
  ...
  const [verifyBmpRoundId,setVerifyBmpRoundId] = useState(null);
  ...
  const doVerify = async () => {
    ...
    try {
      // ── Multiplayer Bingo round lookup ────────────────────────────────────
      if (verifyBmpRoundId !== null && BINGO_MP) {
        const rid = BigInt(verifyBmpRoundId);
        ...
        if (seeded) {
          try { seed = await pub.readContract({address:BINGO_MP, abi:BMP_ABI, functionName:"getRandomSeed", args:[rid]}); } catch {}
          try { chainDrawn = await pub.readContract({address:BINGO_MP, abi:BMP_ABI, functionName:"getDrawnNumbers", args:[rid]}); } catch {}
          if (seed) {
            computedDrawn = computeDrawnNumbersFromSeed(seed);
            if (chainDrawn.length > 0) drawsMatch = chainDrawn.every((n,i) => Number(n) === computedDrawn[i]);
          }
        }
        setVerifyResult({
          gameType:"bingo-mp", roundId:rid.toString(),
          entryFee:r[0], prizePool:r[4], mode:Number(r[5]),
          state, playerCount:Number(r[7]), maxPlayers:Number(r[1]),
          winners:r[8], seeded, entropySeqNum:r[10].toString(),
          startTime:Number(r[3]), seed, chainDrawn:chainDrawn.map(Number),
          computedDrawn, drawsMatch,
        });
        setVerifyLoading(false);
        return;
      }
      // ── Single-bet lookup (coinflip / diceroll / bingo) ───────────────────
  ...
  const IcoGames   = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="6" y1="12" x2="10" y2="12"/><line x1="8" y1="10" x2="8" y2="14"/><circle cx="15" cy="11" r="1" fill="currentColor" stroke="none"/><circle cx="17" cy="13" r="1" fill="currentColor" stroke="none"/><path d="M17.32 5H6.68a4 4 0 0 0-3.978 3.59c-.006.052-.01.101-.017.152C2.604 9.416 2 14.456 2 16a3 3 0 0 0 3 3c1 0 1.5-.5 2-1l1.414-1.414A2 2 0 0 1 9.828 16h4.344a2 2 0 0 1 1.414.586L17 18c.5.5 1 1 2 1a3 3 0 0 0 3-3c0-1.545-.604-6.584-.685-7.258-.007-.05-.011-.1-.017-.151A4 4 0 0 0 17.32 5z"/></svg>;
  ...
  const IcoBTC     = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2"/><path d="M9 8h4.5a2 2 0 0 1 0 4H9m0 0h5a2 2 0 0 1 0 4H9M9 8V7m0 9v1M12 8V7m0 9v1" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg>;
        ...
        <div style={{minHeight:"calc(100vh - 73px)",display:"flex",flexDirection:"column",alignItems:"center",padding:"0 0 60px"}}>
          <div style={{width:"100%",maxWidth:520,padding:"0 20px"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",background:"var(--s2)",border:"1px solid var(--bd)",borderRadius:14,overflow:"hidden",margin:"20px 0"}}>
              {[{l:"LIVE GAMES",v:"4"},{l:"BLOCKCHAIN",v:"Base"},{l:"CURRENCY",v:"USDC"}].map(({l,v},i)=>(
            ...
            <div style={{padding:"18px 20px",background:"var(--s2)",border:"1px solid var(--bd)",borderRadius:14,display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <a className="social-link" title="X / Twitter (temporarily suspended)" aria-disabled="true" style={{opacity:0.35,cursor:"not-allowed",pointerEvents:"none"}}>
        ...
        <div style={{position:"fixed",inset:0,zIndex:100,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
          ...
          <div className="fi" style={{position:"relative",background:"var(--bg)",border:"1px solid var(--bd)",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:520,padding:"24px 20px 36px",zIndex:101}}>
            ...
            {[
              ...
              {id:"predict",  label:"BTC Predict", desc:"1-min price prediction market", Icon:IcoBTC},
          ...
          <div className="fi" style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {[
                ...
                {id:"predict", label:"BTC Predict",mult:null,     desc:"1-min UP/DOWN prediction",    Icon:IcoBTC,   color:"#F7931A", bg:"rgba(247,147,26,0.12)"},
          ...
          <div>
            ...
            <div style={{display:"flex",borderBottom:"1px solid var(--bd)",marginBottom:16,overflowX:"auto"}}>
              {[
                ...
                {id:"predict", Icon:IcoBTC,   label:"BTC Predict"},
              ].map(({id,Icon,label})=>(
            ...
            {/* ── BTC Predict ── */}
            {tab==="predict" && isConnected && authed && (
              <BTCPredict address={address}/>
            )}
          </div>
        )}
  ...
  <path d="M11.5 19.4L18 23.2V15.6L11.5 19.4Z" fill="white" fillOpacity="0.6"/>
              ...
              <>
                ...
                <div className="card" style={{display:"flex",flexDirection:"column",gap:12}}>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    <input className="inp" placeholder="Sequence number or Round # (e.g. 73911)" value={verifySeq} onChange={e=>{setVerifySeq(e.target.value);setVerifyBmpRoundId(null);setVerifyResult(null);setVerifyErr(null);}} onKeyDown={e=>e.key==="Enter"&&doVerify()} style={{flex:1,fontSize:14}}/>
